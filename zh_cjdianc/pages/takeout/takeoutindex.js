...
Page({
  ...
  gwcdec: function(t) {
    for (var s = this, o = this.data.dishes, i = t.currentTarget.dataset.goodid, a = t.currentTarget.dataset.id, e = 0, r = o.length; e < r; e++)
      for (var n = 0, d = o[e].good.length; n < d; n++)
        if (o[e].good[n].id == i) {
          var c = 1;
            ...
            success: function(t) {
              if (console.log(t), "1" == t.data) {
                for (var a = 0; a < o.length; a++)
                  for (var e = 0; e < o[a].good.length; e++) o[a].good[e].id == i && (o[a].good[e].quantity = o[a].good[e].quantity - c);
                s.setData({
                  dishes: o
                }), s.gwcreload();
              }
      ...
      success: function(t) {
        if (console.log(t), "1" == t.data) {
          for (var a = 0, e = r.length; a < e; a++)
            for (var s = 0, o = r[a].good.length; s < o; s++) r[a].good[s].id == n && r[a].good[s].quantity++;
          i.setData({
            dishes: r
          }), i.gwcreload();
  ...
  cartdec: function(t) {
    for (var a = t.currentTarget.dataset.itemIndex, e = t.currentTarget.dataset.parentindex, i = this.data.dishes, s = this.data.cart_list.res, r = t.currentTarget.dataset.goodid, n = this, o = this.data.dishes[e].good[a], d = (wx.getStorageSync("users").id,
        this.data.store_id, 0), c = s.length; d < c; d++)
      if (s[d].good_id == r) {
        var l = 1;
          ...
          success: function(t) {
            if (console.log(t), "1" == t.data) {
              for (var a = 0, e = i.length; a < e; a++)
                for (var s = 0, o = i[a].good.length; s < o; s++) i[a].good[s].id == r && (i[a].good[s].quantity = i[a].good[s].quantity - l);
              n.setData({
                dishes: i
              }), n.gwcreload();
            }
    ...
    "0" == o.start_num || n.isInArray(s, r) || (g = Number(o.start_num)), console.log(g),
      wx.showLoading({
        ...
        success: function(t) {
          if (console.log(t), "1" == t.data) {
            for (var a = 0, e = i.length; a < e; a++)
              for (var s = 0, o = i[a].good.length; s < o; s++) i[a].good[s].id == r && (i[a].good[s].quantity = g + i[a].good[s].quantity);
            n.setData({
              dishes: i
            }), n.gwcreload();
          } else "超出库存!" == t.data ? wx.showModal({
    ...
    console.log(d, c, l), app.util.request({
      ...
      success: function(t) {
        ...
        var a = t.data.spec,
          e = t.data.name;
        for (var s in a)
          for (var o in a[s].spec_val) a[s].spec_val[o].checked = 0 == o;
        g.setData({
          gg: a,
          spname: e
        }), console.log(a);
        ...
        for (var s in a) {
          var n = !1;
          for (var o in a[s].spec_val)
            if (a[s].spec_val[o].checked) {
  ...
  attrClick: function(t) {
    var a = this,
      ...
      i = a.data.gg;
    for (var r in console.log(s, o, i), i)
      if (i[r].spec_id == s)
        for (var n in i[r].spec_val) i[r].spec_val[n].spec_val_id == o ? i[r].spec_val[n].checked = !0 : i[r].spec_val[n].checked = !1;
    a.setData({
      gg: i
    });
    var d = [],
    ...
    for (var r in i) {
      var l = !1;
      for (var n in i[r].spec_val)
        if (i[r].spec_val[n].checked) {
  ...
  ggaddcart: function() {
    var t = this.data.itemIndex,
      ...
      i = this.data.dishes,
      r = (this.data.dishes[a].good[t],
        this.data.cart_list.res, this),
      n = this.data.gginfo,
      e = wx.getStorageSync("users").id,
      s = this.data.gg,
    ...
    for (var l in s) {
      var g = !1;
      for (var u in s[l].spec_val)
        if (s[l].spec_val[u].checked) {
    ...
    c && (wx.showLoading({
      ...
      success: function(t) {
        if (console.log(t), "1" == t.data) {
          for (var a = 0, e = i.length; a < e; a++)
            for (var s = 0, o = i[a].good.length; s < o; s++) i[a].good[s].id == n.good_id && i[a].good[s].quantity++;
          r.setData({
            dishes: i
          ...
          o[r].good = t.data, app.util.request({
            ...
            success: function(t) {
              ...
              for (var a = t.data.res, e = 0; e < a.length; e++)
                for (var s = 0; s < o[r].good.length; s++) a[e].good_id == o[r].good[s].id && (o[r].good[s].quantity = o[r].good[s].quantity + Number(a[e].num));
  ...
  scroll: function(t) {
    this.data.dishes;
    for (var a = this.data.dataheith, e = (this.data.catalogSelect, t.detail.scrollTop), s = 0; s < a.length; s++)
      if (e <= a[s]) {
        this.setData({
          catalogSelect: s,
          toType: "type" + (s - 2)
        ...
        success: function(t) {
          ...
          o[s.currentTarget.dataset.itemIndex].good = t.data, i.setData({
            ...
            success: function(t) {
              console.log(t);
              for (var a = t.data.res, e = 0; e < a.length; e++)
                for (var s = 0; s < o[d].good.length; s++) a[e].good_id == o[d].good[s].id && (o[d].good[s].quantity = o[d].good[s].quantity + Number(a[e].num));
              console.log(o), i.setData({
                dishes: o
              });
            }
        ...
        t.confirm ? (console.log("用户点击确定"), wx.showLoading({
          ...
          success: function(t) {
            if (console.log(t.data), "1" == t.data) {
              for (var a = 0, e = r.length; a < e; a++)
                for (var s = 0, o = r[a].good.length; s < o; s++) r[a].good[s].quantity = 0;
              i.setData({
                dishes: r,
                share_modal_active: !1
              }), i.gwcreload();
  ...
  chongzgi: function () {
    ...
    wx: wx.navigateTo({
      url: '/zh_cjdianc/pages/seller/index?scene=' + scene
    })
  },
  onLoad: function(e) {
    console.log("options", e), app.setNavigationBarColors(this);
                  ...
                  success: function(t) {
                    ...
                    for (var a = t.data.res, e = 0; e < a.length; e++)
                      for (var s = 0; s < i.length; s++)
                        for (var o = 0; o < i[s].good.length; o++) a[e].good_id == i[s].good[o].id && (i[s].good[o].quantity = i[s].good[o].quantity + Number(a[e].num));
                    console.log(i), c.setData({
                      cart_list: t.data,
                      dishes: i,
                      isloading: !1
              ...
              app.util.request({
                ...
                success: function(t) {
                  ...
                  r[0].good = t.data, c.setData({
                    ...
                    success: function(t) {
                      console.log(t);
                      for (var a = t.data.res, e = 0, s = a.length; e < s; e++)
                        for (var o = 0; o < r.length; o++)
                          for (var i = 0; i < r[o].good.length; i++) a[e].good_id == r[o].good[i].id && (r[o].good[i].quantity = r[o].good[i].quantity + Number(a[e].num));

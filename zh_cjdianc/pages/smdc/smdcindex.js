...
Page({
      ...
      success: function(t) {
        if (console.log(t), "1" == t.data) {
          for (var e = 0; e < s.length; e++)
            for (var a = 0; a < s[e].good.length; a++) s[e].good[a].id == n && s[e].good[a].quantity--;
          o.setData({
            dishes: s
          }), o.gwcreload();
        }
          ...
          success: function(t) {
            if (console.log(t), "1" == t.data) {
              for (var e = 0; e < o.length; e++)
                for (var a = 0; a < o[e].good.length; a++) o[e].good[a].id == n && o[e].good[a].quantity--;
              i.setData({
                dishes: o
              }), i.gwcreload();
            }
  ...
  cartadd: function(t) {
    ...
    console.log(e, a, s, i, d, r), wx.showLoading({
      ...
      success: function(t) {
        if (console.log(t), "1" == t.data) {
          for (var e = 0; e < o.length; e++)
            for (var a = 0; a < o[e].good.length; a++) o[e].good[a].id == s && o[e].good[a].quantity++;
          n.setData({
            dishes: o
    ...
    console.log(r, c, l), app.util.request({
      ...
      success: function(t) {
        ...
        var e = t.data.spec,
          a = t.data.name;
        for (var o in e)
          for (var s in e[o].spec_val) e[o].spec_val[s].checked = 0 == s;
        g.setData({
          gg: e,
          spname: a
        }), console.log(e);
        ...
        for (var o in e) {
          var d = !1;
          for (var s in e[o].spec_val)
            if (e[o].spec_val[s].checked) {
  ...
  attrClick: function(t) {
    var e = this,
      ...
      n = e.data.gg;
    for (var i in console.log(o, s, n), n)
      if (n[i].spec_id == o)
        for (var d in n[i].spec_val) n[i].spec_val[d].spec_val_id == s ? n[i].spec_val[d].checked = !0 : n[i].spec_val[d].checked = !1;
    e.setData({
      gg: n
    });
    var r = [],
    ...
    for (var i in n) {
      var l = !1;
      for (var d in n[i].spec_val)
        if (n[i].spec_val[d].checked) {
  ...
  ggaddcart: function() {
    ...
    for (var l in i) {
      var g = !1;
      for (var u in i[l].spec_val)
        if (i[l].spec_val[u].checked) {
    ...
    console.log("加入购物车", t, e, o, n, a, d, i, r, r.toString()), c && (wx.showLoading({
      ...
      success: function(t) {
        if (console.log(t), "1" == t.data) {
          for (var e = 0; e < o.length; e++)
            for (var a = 0; a < o[e].good.length; a++) o[e].good[a].id == n.good_id && o[e].good[a].quantity++;
          s.setData({
            dishes: o
  ...
  scroll: function(t) {
    ...
    for (var o = t.detail.scrollTop, s = 0; s < e.length; s++)
      if (o <= e[s]) {
        ...
        t.confirm ? (console.log("用户点击确定"), wx.showLoading({
          ...
          success: function(t) {
            if (console.log(t.data), "1" == t.data) {
              for (var e = 0; e < s.length; e++)
                for (var a = 0; a < s[e].good.length; a++) s[e].good[a].quantity = 0;
              o.setData({
                dishes: s,
                share_modal_active: !1
              }), o.gwcreload();
  ...
  onLoad: function(t) {
    ...
    console.log(decodeURIComponent(t.scene), e), app.setNavigationBarColors(this), "undefined" != decodeURIComponent(t.scene) && this.setData({
      store_id: e[1],
      tableid: e[0]
    ...
    app.util.request({
      ...
      success: function(t) {
        ...
        console.log("当前的系统时间为" + u), console.log("商家的营业时间从" + a + "至" + o, s + "至" + n),
          1 == i ? (l.setData({
            yysjtoggle: !1
          }), console.log("商家正在休息" + i)) : console.log("商家正在营业" + i), a < n ? a < u && u < o || s < u && u < n || s < u && n < s ? (console.log("商家正常营业"),
            l.setData({
              time: 1
            ...
            time: 3,
            yysjtoggle: !1
          })) : n < a && (a < u && u < o || s < u && n < u || u < s && u < n ? (console.log("商家正常营业"),
            l.setData({
              time: 1
            })) : u < a || o < u && u < s ? (console.log("商家还没开店呐，稍等一会儿可以吗？"), l.setData({
            time: 2,
        ...
        success: function(t) {
          ...
          if (0 < t.data.length) {
            ...
            a.good = t.data, a.type_name = "热销", a.id = "0", d.push(a), app.util.request({
              ...
              success: function(t) {
                console.log(t.data);
                for (var n = d.concat(t.data), e = 0; e < n.length; e++)
                  for (var a = 0; a < n[e].good.length; a++) n[e].good[a].quantity = Number(n[e].good[a].quantity);
                console.log(n);
                ...
                console.log(n), l.setData({
                  ...
                  success: function(t) {
                    console.log(t);
                    for (var e = t.data.res, a = 0; a < e.length; a++)
                      for (var o = 0; o < n.length; o++)
                        for (var s = 0; s < n[o].good.length; s++) e[a].good_id == n[o].good[s].id && (n[o].good[s].quantity = n[o].good[s].quantity + Number(e[a].num));
                    console.log(n), l.setData({
                      cart_list: t.data,
                      dishes: n,
                      isloading: !1
            ...
            success: function(t) {
              console.log(t.data);
              for (var n = t.data, e = 0; e < n.length; e++)
                for (var a = 0; a < n[e].good.length; a++) n[e].good[a].quantity = Number(n[e].good[a].quantity);
              console.log(n);
              for (var o = 0, s = [], i = 0; i < n.length; i++) o += 105 * n[i].good.length, s.push(o);
              console.log(n), l.setData({

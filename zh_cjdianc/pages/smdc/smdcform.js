...
Page({
    ...
    onLoad: function(t) {
        ...
        wx.removeStorageSync("note"), app.util.request({
            ...
            success: function(t) {
                console.log(t.data);
                for (var a = [], e = [], o = 0; o < t.data.length; o++) "1" != t.data[o].coupon_type &&("1" == t.data[o].type || "3" == t.data[o].type)&& a.push(t.data[o]), 
    ...
    formSubmit: function(e) {
        ...
        if ("czkzf" == e.detail.value.radiogroup) {
          var b = Number(this.data.userMoney.money), D = Number(c);
          if (console.log(b, D), b < D) return void wx.showToast({
            title: "储值卡余额不足支付",
            icon: "loading"
          });
        ...
        if ("chzf" == e.detail.value.radiogroup) z = 6;
        if ("czkzf" == e.detail.value.radiogroup) z = 5;
        console.log("支付方式", z), "" == n ? wx.showToast({
            ...
            success: function(t) {
                ...
                i.setData({
                    ...
                    title: "支付失败",
                    icon: "loading"
                      })), "czkzf" == e.detail.value.radiogroup && (console.log("余额支付流程"), "已开台" == a ? wx.showModal({
                        title: "提示",
                        content: "对不起，此桌已开台"
                      }) : "下单失败" != a ? (i.setData({
                        mdoaltoggle: !1
